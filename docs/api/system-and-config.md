...
| `libraryFolderRules` | What a channel folder holds: `[{ channelId, path, kind }]` with `kind` `movie`, `series` or `ignore` (see [library.md](library.md#folder-rules)). PATCH replaces the whole list. |
